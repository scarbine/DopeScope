...
export const MicroscopeForm = () => {
  ...
  const handleSave = (e) => {
    e.preventDefault();
    if (scopeId) {
      updateMicroscope(microscope).then(
        history.push(`/microscope/${microscope.id}`)
      );
    } else {
      getUserByFirebaseId(currentUser.l).then(setUser);
      addMicroscope(microscope).then(history.push("/microscope"));
    }
  };
      ...
      <div className="form-container">
        ...
        <Form className="new-scope-form-wrapper">
          ...
          <FormGroup className="image-upload-field">
            <Label for="slideImageUrl">Uploade Scope Image</Label>
            <br></br>
            <Input
              onChange={(event) => {
          ...
          <FormGroup>
            {scopeId ? (
              <Button className="scope-btn" onClick={handleSave}>
                Update Scope
              </Button>
            ) : (
              <Button className="scope-btn" onClick={handleSave}>
